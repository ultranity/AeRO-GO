...
func (m *MibTCPRow) LocalSock() *SockAddr  { return m.LocalAddr.Sock() }
func (m *MibTCPRow) RemoteSock() *SockAddr { return m.RemoteAddr.Sock() }
func (m *MibTCPRow) SockState() SkState    { return SkState(m.State) }
func (m *MibTCPRow) UID() uint32           { return uint32(m.WinPid) }
...
func (t *MibTCPTable) Rows() []MibTCPRow {
	var s []MibTCPRow
...
// MibTCP6Row structure contains information that describes an IPv6 TCP
// connection.
...
func (m *MibTCP6Row) LocalSock() *SockAddr  { return m.LocalAddr.Sock() }
func (m *MibTCP6Row) RemoteSock() *SockAddr { return m.RemoteAddr.Sock() }
func (m *MibTCP6Row) SockState() SkState    { return SkState(m.State) }
func (m *MibTCP6Row) UID() uint32           { return uint32(m.WinPid) }

// MibTCP6Table structure contains a table of IPv6 TCP connections on the
// local computer.
...
func (t *MibTCP6Table) Rows() []MibTCP6Row {
	var s []MibTCP6Row
	hdr := (*reflect.SliceHeader)(unsafe.Pointer(&s))
	hdr.Data = uintptr(unsafe.Pointer(&t.Table[0]))
	hdr.Len = int(t.NumEntries)
...
// MibUDP4RowOwnerPID structure contains an entry from the User Datagram
...
// the UDP endpoint
type MibUDP4RowOwnerPID struct {
	WinSock
	WinPid
}

func (m *MibUDP4RowOwnerPID) LocalSock() *SockAddr  { return m.Sock() }
func (m *MibUDP4RowOwnerPID) RemoteSock() *SockAddr { return &SockAddr{net.IPv4zero, 0} }
func (m *MibUDP4RowOwnerPID) SockState() SkState    { return Close }
func (m *MibUDP4RowOwnerPID) UID() uint32           { return uint32(m.WinPid) }

// MibUDP4TableOwnerPID structure contains the User Datagram Protocol (UDP)
...
// endpoint.
type MibUDP4TableOwnerPID struct {
	NumEntries uint32
	Table      [1]MibUDP4RowOwnerPID
}

func (t *MibUDP4TableOwnerPID) Rows() []MibUDP4RowOwnerPID {
	var s []MibUDP4RowOwnerPID
	hdr := (*reflect.SliceHeader)(unsafe.Pointer(&s))
	hdr.Data = uintptr(unsafe.Pointer(&t.Table[0]))
	hdr.Len = int(t.NumEntries)
	hdr.Cap = int(t.NumEntries)
...
func rawGetTCPTable(proc uintptr, tab unsafe.Pointer, size *uint32, order bool) error {
	var oint uintptr
	if order {
		oint = 1
	}
...
func getTCPTable(proc uintptr, order bool) ([]byte, error) {
	var (
		size uint32
		buf  []byte
	)
	...
	// determine size
	err := rawGetTCPTable(proc, unsafe.Pointer(nil), &size, false)
	if err != nil && err != errInsuffBuff {
		return nil, err
	}
	buf = make([]byte, size)
	table := unsafe.Pointer(&buf[0])
	err = rawGetTCPTable(proc, table, &size, true)
	if err != nil {
		return nil, err
	}
	return buf, nil
...
func GetTCPTable(order bool) (*MibTCPTable, error) {
	b, err := getTCPTable(procGetTCPTable2.Addr(), true)
	if err != nil {
		return nil, err
	}
	return (*MibTCPTable)(unsafe.Pointer(&b[0])), nil
}

// GetTCP6Table function retrieves the IPv6 TCP connection table
func GetTCP6Table(order bool) (*MibTCP6Table, error) {
	b, err := getTCPTable(procGetTCP6Table2.Addr(), true)
	if err != nil {
		return nil, err
	}
	return (*MibTCP6Table)(unsafe.Pointer(&b[0])), nil
}
...
func GetUDPTableOwnerPID(order bool) (*MibUDP4TableOwnerPID, error) {
	b, err := GetExtendedUDPTable(true, syscall.AF_INET, UDPTableOwnerPID)
	if err != nil {
		return nil, err
	}
	return (*MibUDP4TableOwnerPID)(unsafe.Pointer(&b[0])), nil
}
...
type winSockTable[E winSockEnt] interface {
	Rows() []E
}
...
type tableGetterFunc[E winSockEnt, K winSockTable[E]] func(order bool) (K, error)

func getSocks[E winSockEnt, K winSockTable[E]](accept AcceptFn, getTable tableGetterFunc[E, K]) ([]SockTabEntry, error) {
	tbl, err := getTable(true)
	if err != nil {
		return nil, err
	}
	var sktab []SockTabEntry
...
//ERROR: E (type MibTCPRow) does not satisfy winSockEnt (method LocalSock has pointer receiver)
//func ossTCPSocks(accept AcceptFn) ([]SockTabEntry, error) {
//	return getSocks(accept, GetTCPTable)
//}

func osPorts(mode string, accept AcceptFn) ([]uint16, error) {
	socks, err := osSocks(mode, accept)
	if err != nil {
		return nil, err
	}
	return ToPorts(socks), nil
}
